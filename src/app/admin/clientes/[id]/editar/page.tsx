...
export default async function EditarClientePage({
  ...
  const { data: cliente } = await supabase
    .from('clientes')
    .select(
      'id, nome_empresa, codigo_interno, apelido, apelidos_extra, cnpj_cpf, tipo, regime_tributario, possui_empregados, obrigado_efd_contribuicoes, emite_notas_fiscais, segmento, responsavel, email, telefone, honorario_valor_mensal, honorario_dia_vencimento, observacoes, status, profile_id'
    )
    .eq('id', id)
    .single()
